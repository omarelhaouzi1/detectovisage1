...
from visparharcascade import *
from visparhog import *
def remplissage_hog_csv_fichier(id,chemin_dossier):
    ...
    for i, img_name in enumerate(os.listdir(chemin_dossier)):
        ...
        if os.path.isfile(chemin):
            ...
            if nwid in df['indice'].values:
                ...
                df.to_csv("C:/Users/ASUS/OneDrive/Desktop/Reconnaissance facial/haarcascade/Bank_of_descriptor.csv", index=False)
...
def remplissageparimg(id,chemin_img):
    df=pd.read_csv("C:/Users/ASUS/OneDrive/Desktop/Reconnaissance facial/haarcascade/Bank_of_descriptor.csv")
    vis=visage(chemin_img)
    vect=vecteur(vis)
    vectstr = " ".join(map(str, vect))
    if id in df['indice'].values:
        print("indice deja existe ...")
       
    else:
       df.loc[len(df)] = {'indice': id, 'vecteur': vectstr}
    df.to_csv("C:/Users/ASUS/OneDrive/Desktop/Reconnaissance facial/haarcascade/Bank_of_descriptor.csv", index=False)
...
# rows = curs.fetchall()
# for row in rows:
#     remplissage_hog_csv_fichier(row[0],row[3])
